...
import { ProbeEmitter, Tracer } from 'k6/x/otel'
...
const pipelineProbeEnabled = (__ENV.PIPELINE_LATENCY_PROBE_ENABLED || '').toLowerCase() === 'true'
const configuredProbeInterval = parseFloat(__ENV.PIPELINE_LATENCY_PROBE_INTERVAL || '1')
const pipelineProbeInterval = Number.isFinite(configuredProbeInterval) && configuredProbeInterval > 0
    ? configuredProbeInterval
    : 1
...
export const options = {
    scenarios: {
        ...
        ...(pipelineProbeEnabled ? {
            pipelineProbe: {
                executor: 'constant-vus',
                exec: 'pipelineProbeScenario',
                vus: 1,
                duration: __ENV.K6_DURATION || '9999h',
            },
        } : {}),
...
const probeEmitter = pipelineProbeEnabled ? new ProbeEmitter() : null

export function pipelineProbeScenario() {
    probeEmitter.emit()
    sleep(pipelineProbeInterval)
}
