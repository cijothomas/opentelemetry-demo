...
// system under test via W3C traceparent headers, emit correlated logs, and
// send pipeline probe events. Register with xk6 as "k6/x/otel".
package xk6otel
...
// Exports surfaces the module constructors to JavaScript.
func (m *ModuleInstance) Exports() modules.Exports {
	return modules.Exports{
		Named: map[string]any{
			"ProbeEmitter": m.newProbeEmitter,
			"Tracer":       m.newTracer,
		},
	}
}

func (m *ModuleInstance) newProbeEmitter(call sobek.ConstructorCall, rt *sobek.Runtime) *sobek.Object {
	initProviders()
	if providerErr != nil {
